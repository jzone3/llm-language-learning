...
- **The old "1,000 free service conversations per month" tier no longer exists** — pricing is per message now.
...
- **Test number stays available** in the same WABA for development — keep pointing your local `.env` at it (or stub sends entirely, see `.agents/skills/testing-vocabtext/SKILL.md`); nothing about it changes when the production number goes live.
