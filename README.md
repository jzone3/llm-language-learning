...
3. Every morning (default 8am local) they get one WhatsApp message: a short quiz of due items (active recall) plus 1–2 new items — a curated mix of high-frequency words, everyday phrases, and common slang, with transliteration for non-Latin scripts. Each new item is followed by a simple generated picture of the concept (WhatsApp image message, captioned with the word) for visual learners.
...
- OpenAI — sentence generation, grading, weekly word picking, cadence optimization, Whisper transcription, word illustrations (`gpt-image-1`)
- Vercel Blob — public store for the per-word illustrations (`src/lib/images.ts`); each word's image is generated once and cached on `Word.imageUrl`
...
- Word images: attach a Vercel Blob store with **public** access (Meta fetches the image by URL) so `BLOB_READ_WRITE_TOKEN` is injected. Optional: `OPENAI_IMAGE_MODEL` (default `gpt-image-1`, low quality, 1024×1024, ~$0.01/image; `dall-e-3` also works), `WORD_IMAGES=off` to disable globally, or `User.wantsImages=false` per user. Image failures are logged and never block the text lesson.
